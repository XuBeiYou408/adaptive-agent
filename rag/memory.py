...
from config import REDIS_TTL, LOCAL_DB_PATH, COMPACTION_THRESHOLD, CONTEXT_MAX_TOKENS
...
async def compact_history(messages: List[Any], llm: Any) -> str:
    ...
    try:
        ...
        logger.warning(f"微压缩生成失败 ({e})，安全退避为截断模式")
        compacted_summary = "[摘要] 历史对话已做摘要降级处理"
