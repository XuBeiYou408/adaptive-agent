...
def retrieve_single(q):  # 负责针对某一个具体的问题，同时去向量数据库和传统的 BM25 里捞数据
    ...
    docs_bm25 = bm25.invoke(q)[:3] if bm25 else []
...
async def zhaohui_and_rerank(inputs ,rerank_limit=60,return_documents=False):
    ...
    queries = question_rewriter(question)
    results = await asyncio.to_thread(
        lambda:list(executor.map(retrieve_single,queries))
    )
    all_docs = [doc for sublist in results for doc in
                sublist]  # 二维数组扁平化:把[[文档A, 文档B], [文档C, 文档D]]平铺成一整条单层列表：[文档A, 文档B, 文档C, 文档D]
